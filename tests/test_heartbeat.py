...
def test_probe_detects_a_live_process_and_growing_log(tmp_path):
    log = tmp_path / "run.log"
    log.write_text("seed 0 epoch 1\n")
    # Our own interpreter is a guaranteed-live process to match on.
    hb = probe("python", log, label="self", stall_after=600.0)
    assert hb.alive
...
def test_probe_marks_stalled_when_log_is_old(tmp_path):
    log = tmp_path / "run.log"
    log.write_text("seed 0 epoch 1\n")
    old = time.time() - 3600
    os.utime(log, (old, old))
    hb = probe("python", log, stall_after=60.0)
